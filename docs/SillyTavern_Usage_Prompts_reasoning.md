...
- Claude also supports adaptive thinking for Opus 4.6+ models, which can be enabled via `claude.enableAdaptiveThinking` in [config.yaml](SillyTavern_Administration_config-yaml.md). When enabled, the Reasoning Effort setting maps to adaptive thinking levels instead of token budgets.
