...
you're building a bundled extension, you can import variables and functions from other JS files.
...
can use the structured outputs feature to ensure the model produces a valid JSON object that adheres to a provided JSON Schema (https://json-schema.org/learn). This is useful for extensions that require structured data, such as state tracking, data classification, etc.
...
`doExtrasFetch()` function allows you to make requests to your SillyTavern Extras API server.
