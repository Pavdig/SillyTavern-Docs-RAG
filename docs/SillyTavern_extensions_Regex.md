...
The Regex extension lets the user automatically detect specific patterns in a string of text (called 'sequences') and apply manipulations (replacements) to them. It can be a powerful tool when used in conjunction with other SillyTavern features such as [Quick Replies or STscript](SillyTavern_For_Contributors_st-script.md), or simply a way to remove certain words from a chat.
