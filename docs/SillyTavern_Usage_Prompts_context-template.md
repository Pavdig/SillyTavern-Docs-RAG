...
special `{{trim}}` macro is supported to remove any newlines that surround it. Use it if you want a part of the text to not be separated from the previous line by a newline (_spaces **are not** trimmed_).
...
following section only applies when **Instruct Mode** is ON.
...
the character's name to the prompt to force the model to complete the message as the character:
