...
The template supports Handlebars syntax, custom text injections or formatting, and any other [macros](SillyTavern_usage_macros.md). See the language reference here: <https://handlebarsjs.com/guide/>
...
6. `{{system}}`: The [system prompt](SillyTavern_Usage_Prompts_advancedformatting.md) OR the character's [main prompt](SillyTavern_Usage_Characters_characterdesign.md) override (if it exists and "Prefer Char. Prompt" is enabled in User Settings).
